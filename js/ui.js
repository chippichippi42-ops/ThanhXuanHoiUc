class UIManager {
    ...
    updateTeamStatus() {
        ...
        const enemyPanel = document.getElementById('enemyTeam');
        
        if (!allyPanel || !enemyPanel) return;
        
        const playerTeam = this.gameState.playerHero.team;
        const allies = this.gameState.heroes.filter(h => h.team === playerTeam);
        const enemies = this.gameState.heroes.filter(h => h.team !== playerTeam);
        ...
        for (const enemy of enemies) {
            if (this.isHeroVisible(enemy)) {
                enemyPanel.appendChild(this.createTeamMemberElement(enemy));
            }
        }
    }
